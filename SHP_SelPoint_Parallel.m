function [SHP]=SHP_SelPoint_Parallel(mlistack,CalWin,Alpha,EstAgr,NumBlocks)
%SHP_SelPoint_Parallel 并行版本的同质像元(SHP)选取函数
%   本函数用于在SAR强度图堆栈上选取统计同质像元(Statistically Homogeneous Pixels)
%   针对BWS算法进行了并行优化，大幅提升计算速度
%
%   用法:
%       [SHP]=SHP_SelPoint_Parallel(mlistack,CalWin,Alpha,EstAgr,NumBlocks)
%   
%   输入参数:
...
disp(['SHP_SelPoint_Parallel 运算完成，耗时 ', num2str(t/60, '%.2f'), ' 分钟']);
disp('完成！');
