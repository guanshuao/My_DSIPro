...
if ismember(1, figure_id)
    ...
    for i = 1:level_num
        ...
        if any(idx(:))
            ...
            mean_TrueCoh(i) = NaN; mean_AdpCoh(i) = NaN; mean_BestEst(i) = NaN;
            mean_ModifiedCoh(i) = NaN; mean_BoxCohMax(i) = NaN;
        end
    end
    
    figure('Name', '不同SHP Level下的各估计方法均值对比', 'Position', [100, 100, 800, 600]);
    ...
    ylabel('相干性均值');
    xlim([1 level_num]);
    title('不同 SHP Level 下各估计方法的相干性均值对比');
...
%% 图二：不同SHPNum Level下的各尺寸窗口估计均值对比。
if ismember(2, figure_id)
    ...
    figure('Name', '不同SHP Level下的各尺寸窗口估计均值对比', 'Position', [150, 150, 800, 600]);
    ...
    ylabel('相干性均值');
    xlim([1 level_num]);
    title('不同尺寸窗口估计在各 SHP Level 下的相干性均值对比');
    legend('Location', 'best');
    grid on;
...
if ismember(3, figure_id)
    ...
    figure('Name', '不同SHP Level下各估计方法之间的比值和差值比较', 'Position', [200, 200, 1000, 500]);
    subplot(1, 2, 1);
    ...
    hold off;
    xlabel('SHP Level'); ylabel('相干性估计比值');
    xlim([1 level_num]); title('不同SHP Level下各估计方法之间的比值对比'); legend('Location', 'best'); grid on;
    ...
    hold off;
    xlabel('SHP Level'); ylabel('相干性估计差值');
    xlim([1 level_num]); title('不同SHP Level下各估计方法之间的差值对比'); legend('Location', 'best'); grid on;
