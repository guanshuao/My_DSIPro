...
% 注意此处改为 SHPNum_level
valid_mask = ~isnan(Stack.SHPNum_level) & (Stack.SHPNum_level >= 1 & Stack.SHPNum_level <= level_num);
groups = Stack.SHPNum_level(valid_mask);
groups = round(groups(:));
...
writetable(T1, 'fig1_shp_data.txt', 'Delimiter', '\t');
...
writetable(T2, 'fig2_shp_data.txt', 'Delimiter', '\t');
...
writetable(T3, 'fig3_shp_data.txt', 'Delimiter', '\t');
...
%% Fig 4
win_labels = []; mean_shp_arr = []; q1_arr = []; q2_arr = []; q3_arr = []; min_arr = []; max_arr = [];
for i = 1:num_wins
    win_val = unique_wins(i);
    idx = (Stack.Best_Win == win_val);
    shp_data = Stack.SHPNum_level(idx);
    shp_data(isnan(shp_data)) = []; % Remove NaNs before percentile calc
    
    win_labels = [win_labels; win_val];
    if isempty(shp_data)
        min_arr = [min_arr; NaN]; q1_arr = [q1_arr; NaN];
        q2_arr = [q2_arr; NaN]; q3_arr = [q3_arr; NaN];
        max_arr = [max_arr; NaN]; mean_shp_arr = [mean_shp_arr; NaN];
    else
        % For boxplots, use 1.5*IQR rule for whiskers instead of absolute min/max
        q1_val = prctile(shp_data, 25);
        q2_val = median(shp_data);
        q3_val = prctile(shp_data, 75);
        iqr_val = q3_val - q1_val;
        
        lower_bound = q1_val - 1.5 * iqr_val;
        upper_bound = q3_val + 1.5 * iqr_val;
        
        w_min = min(shp_data(shp_data >= lower_bound));
        w_max = max(shp_data(shp_data <= upper_bound));
        
        if isempty(w_min), w_min = min(shp_data); end
        if isempty(w_max), w_max = max(shp_data); end
        ...
        mean_shp_arr = [mean_shp_arr; mean(shp_data)];
    end
end
T4 = table(win_labels, min_arr, q1_arr, q2_arr, q3_arr, max_arr, mean_shp_arr);
writetable(T4, 'fig4_shp_boxplot.txt', 'Delimiter', '\t');

%% Fig 5
win_percent_matrix_shp = zeros(level_num, num_wins);
for i = 1:level_num
    level_idx = (Stack.SHPNum_level == i);
    for j = 1:num_wins
        win_val = unique_wins(j);
        win_percent_matrix_shp(i, j) = sum(Stack.Best_Win(level_idx) == win_val);
    end
end
row_sums = sum(win_percent_matrix_shp, 2);
win_percent_matrix_shp = win_percent_matrix_shp ./ row_sums * 100;
win_percent_matrix_shp(isnan(win_percent_matrix_shp)) = 0;

T5 = table(levels);
for j = 1:num_wins
    T5.(sprintf('Win_%d', unique_wins(j))) = win_percent_matrix_shp(:, j);
end
writetable(T5, 'fig5_shp_stacked.txt', 'Delimiter', '\t');

%% Fig 6
mean_win_shp = zeros(level_num, 1);
for i = 1:level_num
    level_idx = (Stack.SHPNum_level == i);
...
T6 = table(levels, mean_win_shp);
writetable(T6, 'fig6_shp_mean_win.txt', 'Delimiter', '\t');


disp('Data exported to txt successfully (SHP). MATLAB processing complete.');
